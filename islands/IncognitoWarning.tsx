...
const isIncognito = signal(true);

export default function IncognitoWarning() {
  if (!IS_BROWSER) return <div></div>;

  detectIncognito().then(({ isPrivate }) => {
    isIncognito.value = isPrivate;
  });

  return !isIncognito.value
    ? (
      <p class="my-4 text-center toast-alert">
        It seems like you are not using incognito. Browser extensions may access
        page the content. Be cautious.
      </p>
    )
    : <div></div>;
        ...
        try {
          ...
          db.onupgradeneeded = function (i) {
            ...
            try {
              ...
              const matchesExpectedError = /BlobURLs are not yet supported/
                .test(message);
